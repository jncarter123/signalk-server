...
module.exports = function(app, saveSecurityConfig, getSecurityConfig) {
  ...
  app.get('/admin/', (req, res) => {
    fs.readFile(path.join(__dirname, '/../node_modules/@signalk/server-admin-ui/public/index.html'),
      (err, indexContent) => {
        if (err) {
          console.error(err)
          res.status(500)
        ...
        res.send(
          indexContent.toString().replace(
            /%ADDONSCRIPTS%/g,
            (app.addons || []).map(moduleInfo => `<script src="/${moduleInfo.name}/remoteEntry.js"></script>`)))
      })
  })
